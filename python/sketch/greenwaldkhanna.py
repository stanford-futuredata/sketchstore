...
class GKArray(object):
    ...
    def add_weighted(self, vals, weights):
        n = len(vals)
        entries = [
            Entry(vals[i], weights[i], 0)
            for i in range(n)
        ]
        self._min = min(self._min, np.min(vals))
        self._max = max(self._max, np.max(vals))
        self._count += np.sum(weights)
    ...
    def rank(self, x):
        return sum([e.g for e in self.entries if e.val <= x])

    def get_dict(self):
        return {e.val: e.g for e in self.entries}
