 ...
 */
public class PatchedCountMinSketch implements IFrequency, Serializable {
    ...
    PatchedCountMinSketch() {
    }

    public PatchedCountMinSketch(int depth, int width, int seed) {
        this.depth = depth;
        this.width = width;
        this.eps = 2.0 / width;
    ...
    public PatchedCountMinSketch(double epsOfTotalCount, double confidence, int seed) {
        // 2/w = eps ; w = 2/eps
    ...
    PatchedCountMinSketch(int depth, int width, long size, long[] hashA, long[][] table) {
        this.depth = depth;
        this.width = width;
        this.eps = 2.0 / width;
        this.confidence = 1 - 1 / Math.pow(2, depth);
    ...
    public boolean equals(Object o) {
        ...
        final PatchedCountMinSketch that = (PatchedCountMinSketch) o;
    ...
    public PatchedCountMinSketch merge(PatchedCountMinSketch other) {
     ...
     */
    public static PatchedCountMinSketch merge(PatchedCountMinSketch... estimators) throws Exception {
        PatchedCountMinSketch merged = null;
        if (estimators != null && estimators.length > 0) {
            ...
            for (PatchedCountMinSketch estimator : estimators) {
                if (estimator.depth != depth) {
                    throw new Exception("Cannot merge estimators of different depth");
                }
                if (estimator.width != width) {
                    throw new Exception("Cannot merge estimators of different width");
                }
                if (!Arrays.equals(estimator.hashA, hashA)) {
                    throw new Exception("Cannot merge estimators of different seed");
                }
            ...
            merged = new PatchedCountMinSketch(depth, width, size, hashA, table);
        }

        return merged;
    }

    public static byte[] serialize(PatchedCountMinSketch sketch) {
    ...
    public static PatchedCountMinSketch deserialize(byte[] data) {
        ByteArrayInputStream bis = new ByteArrayInputStream(data);
        DataInputStream s = new DataInputStream(bis);
        try {
            PatchedCountMinSketch sketch = new PatchedCountMinSketch();
